 ...
 * Uses Belema brand palette on a dark panel — no external animation libs.
 */
export function LoginFinanceHero({ className = "" }) {
  const { brand } = useBrand();
  const green = brand.palette?.brandGreen || brand.theme.loginPrimary || "#00411A";
  const lime = brand.palette?.lime || brand.theme.chart?.[1] || "#CEF445";
  const yellow = brand.palette?.yellow || brand.theme.primary || "#FFD600";
  const darkGreen = "#003014";
  const limeLight = "#e8f5a0";
  const limeMid = "#d4ed7a";
  const mint = "#f0f9d4";
      ...
      <svg viewBox="0 0 420 420" className="h-auto w-full" role="img" aria-label="Financial services illustration">
        <defs>
          <linearGradient id="lfh-card" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor={limeMid} />
          ...
          <linearGradient id="lfh-card-lite" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <stop offset="100%" stopColor={limeMid} />
          </linearGradient>
          <linearGradient id="lfh-mint" x1="0%" y1="0%" x2="0%" y2="100%">
            <stop offset="0%" stopColor={mint} />
            <stop offset="100%" stopColor={limeLight} />
          </linearGradient>
        ...
        <g className="lfh-card-back" filter="url(#lfh-soft)">
          ...
          <rect x="112" y="152" width="72" height="10" rx="5" fill={green} opacity="0.35" />
          <rect x="112" y="172" width="48" height="8" rx="4" fill={green} opacity="0.25" />
        </g>

        <g className="lfh-card-main" filter="url(#lfh-soft)">
          <rect x="132" y="148" width="196" height="124" rx="20" fill="url(#lfh-card)" />
          <rect x="152" y="172" width="36" height="28" rx="6" fill={yellow} opacity="0.9" />
          <rect x="152" y="214" width="110" height="10" rx="5" fill={mint} opacity="0.55" />
          <rect x="152" y="232" width="72" height="8" rx="4" fill={mint} opacity="0.35" />
          <circle cx="292" cy="188" r="14" fill={limeMid} opacity="0.85" />
          <circle cx="310" cy="188" r="14" fill={lime} opacity="0.7" />
          <text x="152" y="268" fill={mint} fontSize="13" fontFamily="system-ui,sans-serif" fontWeight="600" letterSpacing="1.5">
            BELEMA PAY
          </text>
        </g>

        <g transform="translate(286 286)">
          <rect className="lfh-bar" x="0" y="28" width="12" height="36" rx="3" fill={green} />
          <rect className="lfh-bar lfh-bar-2" x="18" y="16" width="12" height="48" rx="3" fill={limeMid} />
          <rect className="lfh-bar lfh-bar-3" x="36" y="8" width="12" height="56" rx="3" fill={lime} />
          <rect className="lfh-bar lfh-bar-4" x="54" y="0" width="12" height="64" rx="3" fill={yellow} />
        </g>

        <g className="lfh-coin">
          <circle cx="72" cy="168" r="22" fill="url(#lfh-mint)" />
          <circle cx="72" cy="168" r="16" fill="none" stroke={green} strokeWidth="2" opacity="0.55" />
          <text x="72" y="174" textAnchor="middle" fill={green} fontSize="16" fontFamily="system-ui,sans-serif" fontWeight="700">₦</text>
        </g>
        <g className="lfh-coin-2">
          <circle cx="348" cy="152" r="18" fill={limeMid} />
          <text x="348" y="158" textAnchor="middle" fill={darkGreen} fontSize="14" fontFamily="system-ui,sans-serif" fontWeight="700">$</text>
        </g>
        <g className="lfh-coin-3">
          <circle cx="86" cy="292" r="16" fill={lime} />
          <text x="86" y="297" textAnchor="middle" fill={green} fontSize="12" fontFamily="system-ui,sans-serif" fontWeight="700">€</text>
        </g>

        <g transform="translate(178 292)" filter="url(#lfh-soft)">
          <ellipse className="lfh-shield-glow" cx="28" cy="34" rx="34" ry="18" fill={green} opacity="0.35" />
          <path
            ...
            strokeWidth="2"
          />
          <path d="M20 30 L26 36 L38 22" fill="none" stroke={green} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
        </g>
      </svg>
    </div>
  );
